...
import { Mic, MicOff, PhoneOff, Clock, Bot, Circle, Square, Download, Play } from "lucide-react";
...
type Props = {
  ...
  enableRecording?: boolean; // New prop to enable call recording
};
...
const AutoConnectCall = ({
  ...
  webinar,
  userId,
  autoStart = true, // Auto-start by default for backwards compatibility
  enableRecording = false, // Recording disabled by default
}: Props) => {
  const [callStatus, setCallStatus] = useState(autoStart ? CallStatus.CONNECTING : CallStatus.FINISHED);
  ...
  const startRecording = async () => {
    if (!enableRecording || !refs.current.audioStream) return;

    try {
      const mediaRecorder = new MediaRecorder(refs.current.audioStream);
  ...
  // Call setup & cleanup
  useEffect(() => {
    // Start the call immediately on mount only if autoStart is enabled
    if (autoStart) {
      startCall();
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] bg-background">
      ...
      <div className="flex-1 flex flex-col md:flex-row p-4 gap-4 relative">
        ...
        {callStatus === CallStatus.FINISHED && !autoStart && recordedChunks.length === 0 && (
          <div className="absolute inset-0 bg-background/90 flex items-center justify-center flex-col gap-4 z-20">
            <h3 className="text-xl font-medium">Ready to Start Call</h3>
            <p className="text-muted-foreground">Click the button below to begin</p>
            <Button onClick={startCall} size="lg" className="bg-gradient-to-r from-indigo-500 to-rose-500">
        ...
        {callStatus === CallStatus.FINISHED && (autoStart || recordedChunks.length > 0) && (
          <div className="absolute inset-0 bg-background/90 flex items-center justify-center flex-col gap-4 z-20">
            ...
            <p className="text-muted-foreground">
              {recordedChunks.length > 0 ? "Recording available below" : "Time limit reached"}
            </p>
      ...
      <div className="bg-card border-t border-border p-4">
        <div className="max-w-3xl mx-auto flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-4">
            {/* Recording controls */}
            {enableRecording && callStatus === CallStatus.ACTIVE && (
              <button
