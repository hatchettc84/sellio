...
export const getWebinarAttendance = async (
  ...
  try {
    ...
    for (const type of Object.values(AttendedTypeEnum)) {
      ...
      const countItem = attendanceCounts.find((item: { attendedType: AttendedTypeEnum; _count: { attendedType: number } }) => {
    ...
    if (options.includeUsers) {
      ...
      for (const type of Object.values(AttendedTypeEnum)) {
        ...
        if (result[type].count > 0) {
          ...
          result[type].users = attendances.map((attendance: { user: { id: string; name: string | null; email: string; callStatus: CallStatusEnum | null; createdAt: Date; updatedAt: Date }; joinedAt: Date | null }) => ({
